...
const checks = [
  ...
  ['drag and drop upload UI', ui.includes('id="dropZone"') && ui.includes("dropZone.addEventListener('drop'")],
  ['automatic title and slug', ui.includes('slugFromFilename') && ui.includes("querySelector('title')")],
  ['lazy thumbnail previews', ui.includes('site-thumb-frame') && ui.includes('loading="lazy"')],
  ['thumbnail frame restricted to admin host', index.includes('publicCsp(mode, env.ADMIN_HOST)') && index.includes('frameAncestor')],

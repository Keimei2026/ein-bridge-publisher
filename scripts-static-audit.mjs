...
const checks = [
  ...
  ['stable canonical public URL', index.includes('const contentPath = `/p/${encodeURIComponent(slug)}/content/${encodeURIComponent(site.currentRevision)}`') && index.includes('\"cache-control\": \"no-store\"')],
  ['legacy revision URL redirects to canonical', index.includes('const location = `/p/${encodeURIComponent(slug)}`')],
  ['published content can be framed same-origin', index.includes("frame-ancestors 'self'")],
